...
const RegistroBeni = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [assetsRes, locationsRes] = await Promise.all([
        supabase.from('beni_mobili').select('*').order('created_at', { ascending: false }),
  ...
  const applyFiltersAndSort = useCallback(() => {
    ...
    if (filters.minValue) tempAssets = tempAssets.filter(asset => (asset.valore_unitario || 0) >= parseFloat(filters.minValue));
    if (filters.maxValue) tempAssets = tempAssets.filter(asset => (asset.valore_unitario || 0) <= parseFloat(filters.maxValue));
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1200px]">
                  ...
                  <tbody className="divide-y divide-border">
                    {loading ? (
                      <tr><td colSpan={tableHeaders.length} className="px-6 py-12 text-center text-muted-foreground">Caricamento dati...</td></tr>
