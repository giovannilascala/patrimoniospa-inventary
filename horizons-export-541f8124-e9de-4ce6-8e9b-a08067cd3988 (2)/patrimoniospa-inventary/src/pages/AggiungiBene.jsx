...
const AggiungiBene = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (imageFile) {
        ...
        if (uploadError) {
            toast({ variant: "destructive", title: "Errore Upload", description: uploadError.message });
            setIsSubmitting(false);
            return;
        }

        const { data: { publicUrl } } = supabase.storage.from('img').getPublicUrl(filePath);
        imageUrl = publicUrl;
    }
    ...
    if (insertError) {
      console.error('Error adding asset:', insertError);
      toast({
        variant: 'destructive',
        title: 'Errore',
        description: 'Impossibile aggiungere il bene. Riprova.',
      });
    } else {
      toast({
        title: 'Successo!',
          ...
          <form onSubmit={handleSubmit} className="bg-card text-card-foreground p-8 rounded-xl shadow-sm border space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2">
                    ...
                    <input type="text" id="descrizione" name="descrizione" value={formData.descrizione} onChange={handleInputChange} required className="w-full px-4 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500" />
                </div>
                 <div>
                    <label htmlFor="quantita" className="block text-sm font-medium text-muted-foreground mb-1">Quantità</label>
                    <input type="number" id="quantita" name="quantita" value={formData.quantita} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" min="1"/>
                </div>
            </div>
            
            <div>
                <label htmlFor="note" className="block text-sm font-medium text-muted-foreground mb-1">Note Aggiuntive</label>
                <textarea id="note" name="note" value={formData.note} onChange={handleInputChange} rows="3" className="w-full px-4 py-2 border rounded-lg"></textarea>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 border-t pt-8">
                <div>
                    ...
                    <input type="number" step="0.01" id="valore_unitario" name="valore_unitario" value={formData.valore_unitario} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                </div>
                <div>
                    <label htmlFor="codice_bilancio" className="block text-sm font-medium text-muted-foreground mb-1">Codice Bilancio</label>
                    <input type="text" id="codice_bilancio" name="codice_bilancio" value={formData.codice_bilancio} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                </div>
                <div>
                    <label htmlFor="dipartimento" className="block text-sm font-medium text-muted-foreground mb-1">Dipartimento</label>
                    <input type="text" id="dipartimento" name="dipartimento" value={formData.dipartimento} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                </div>
                 <div>
                    <label htmlFor="sede" className="block text-sm font-medium text-muted-foreground mb-1">Sede</label>
                    <select id="sede" name="sede" value={formData.sede} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg">
                ...
                <div>
                    ...
                    <input type="text" id="stanza" name="stanza" value={formData.stanza} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                </div>
                <div>
                    <label htmlFor="stato" className="block text-sm font-medium text-muted-foreground mb-1">Stato</label>
                    <select id="stato" name="stato" value={formData.stato} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg">
                ...
                <div>
                    ...
                    <input type="date" id="data_acquisizione" name="data_acquisizione" value={formData.data_acquisizione} onChange={handleInputChange} className="w-full px-4 py-2 border rounded-lg" />
                </div>
            </div>
