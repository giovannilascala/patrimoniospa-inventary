...
const DettaglioBene = () => {
  ...
  const handleDelete = async () => {
    // First, try to delete associated files from storage
    // We do this best-effort, if it fails, we still try to delete the DB record
    try {
        if (asset.img_url) {
            const urlPath = new URL(asset.img_url).pathname;
            // Assuming Supabase storage URL format: /storage/v1/object/public/[bucket]/[path]
            // We need to extract [path] relative to bucket root.
            const pathParts = urlPath.split('/img/');
            if (pathParts.length > 1) {
                await supabase.storage.from('img').remove([pathParts[1]]);
            }
        }
        if (asset.documenti_url && asset.documenti_url.length > 0) {
            const paths = asset.documenti_url.map(url => {
                 const urlPath = new URL(url).pathname;
                 const parts = urlPath.split('/documents/');
        ...
        console.warn('Could not delete associated files, proceeding with DB deletion:', storageError);
    }
  ...
  const handleFileUpload = async (e, type) => {
    ...
    // Use a simpler, safe filename to avoid encoding issues
  ...
  const detailItems = useMemo(() => [
    ...
    { icon: Euro, label: 'Valore Unitario (€)', value: `€${parseFloat(asset?.valore_unitario || 0).toLocaleString('it-IT')}`, key: 'valore_unitario', type: 'number', step: '0.01' },
                            ...
                            <div className="w-full aspect-square rounded-lg bg-secondary border-2 border-dashed flex items-center justify-center overflow-hidden mb-4">
                                {asset.img_url ? (
                                    <div className="relative group w-full h-full">
                                        ...
                                        <button type="button" onClick={() => handleFileDelete(asset.img_url, 'image')} className="absolute top-2 right-2 p-1.5 bg-red-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                {(asset.documenti_url && asset.documenti_url.length > 0) ? (
                                    asset.documenti_url.map((doc, i) => (
                                        <div key={i} className="flex items-center justify-between p-2 bg-secondary rounded-lg border">
                                            ...
                                            <button type="button" onClick={() => handleFileDelete(doc, 'document')} className="p-1 text-red-500 hover:text-red-700">
                    ...
                    <div className="lg:col-span-2 bg-card p-8 rounded-xl shadow-sm border">
                        {isEditing ? (
                            <>
                                ...
                                <input name="descrizione" value={editData.descrizione || ''} onChange={handleInputChange} className="text-3xl font-bold text-foreground bg-transparent w-full mb-4 border-b-2 pb-2" required />
